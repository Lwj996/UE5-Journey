# 第 1 周：精确到小时的计划（修订版）· ⚠️ DEPRECATED（已作废）

> **2026-06-22 作废**：本表是"找海外工作"路径下的第 1 周计划（4/27-5/3）。
> 阶段 1 已完成（见 [`journal/`](../journal/) Day 03-Day 09 日志）；后续节奏已两次重定（5/7 cadence rebuild → 6/22 项目驱动双轨）。
> **本文件保留不删**——记录起步周的真实执行表，留给未来对照。
>
